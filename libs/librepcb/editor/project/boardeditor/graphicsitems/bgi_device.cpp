...
void BGI_Device::updateBoardSide() noexcept {
  ...
  for (int i = 0; i < std::min<int>(circles.count(), mCircleGraphicsItems.count());
       ++i) {
  ...
  for (int i = 0; i < std::min<int>(polygons.count(), mPolygonGraphicsItems.count());
       ++i) {
...
void BGI_Device::updateHoleStopMaskOffsets() noexcept {
  for (int i = 0; i < std::min<int>(mDevice.getLibFootprint().getHoles().count(),
...
void BGI_Device::updateZoneLayers() noexcept {
  ...
  for (int i = 0; i < std::min<int>(zones.count(), mZoneGraphicsItems.count());
       ++i) {
